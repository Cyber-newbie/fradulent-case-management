...
export class ParticipantRepository extends BaseRepository<Participant> implements IRepository<Participant> {
    ...
    async create(data: Participant): Promise<Participant[]> {
        ...
        this.query = `INSERT INTO participants (institute_name, type, email, phone_number, address, country, status) 
            VALUES ('${data.getInstituteName()}', '${data.getType()}', '${data.getEmail()}', 

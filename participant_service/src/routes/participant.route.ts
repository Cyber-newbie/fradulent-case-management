...
import { AccountStatus, ActionValues, UserRole } from "../utils/Enums";
...
class ParticipantRouter {
    ...
    constructor(){  
        ...
        this.router.post('/account/json-data', this.participantController.uploadAccountDataJson)
        this.router.post('/account/upload-data', multer.single('data'), this.participantController.uploadAccountDataFile)

        this.router.post('/transaction/json-data',  this.participantController.uploadTransactionDataJson)
        this.router.post('/transaction/upload-data', multer.single('data'), this.participantController.uploadTransactionDataFile)
        
        this.router.post<string, null, IResponseBody, IUserRoleDto>('/register/user', 
